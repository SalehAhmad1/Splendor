...
export async function GET(req: NextRequest) {
    try {
        ...
        if (!fs.existsSync(genderDir)) {
            ...
            return NextResponse.json({ error: 'Women\'s wear directory not found' }, { status: 404 });
        }
        ...
        return NextResponse.json({ 
            error: 'Failed to fetch women\'s clothing data',
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (newClothingItem.for !== 'women') {
            return NextResponse.json({ error: 'Wrong API' }, { status: 400 });
        }
